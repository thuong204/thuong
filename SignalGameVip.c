...
int main() {
    ...
    // Set up the pause handler for SIGTSTP (Ctrl + Z)
...
void fork_process() {
    ...
    if (pid < 0) {
        ...
        while (1) {
            // Check for pause
            while (paused) {
                printf("Game is paused (Child). Press Ctrl + Z to resume.\n");
                sleep(1);
            }
        ...
        while (1) {
            // Check for pause
            while (paused) {
                printf("Game is paused (Parent). Press Ctrl + Z to resume.\n");
                sleep(1);
            }
...
// Handle SIGTSTP signal for pause/resume
void pause_handler(int signo) {
    (void)signo; // Unused
    paused = !paused; // Toggle the pause state
    if (paused) {
        printf("Game paused. Press Ctrl + Z to resume.\n");
    } else {
        printf("Game resumed.\n");
    }
}
...
void print_instructions() {
    ...
    printf("6. Press Ctrl + Z to pause/resume the game.\n");
    printf("\nEnjoy the game!\n");
}
